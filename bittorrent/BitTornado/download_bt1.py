...
#    ('ipv6_enabled', autodetect_ipv6(),
    ...
    ###### GROUP VOD ######
    ('delay', '',
        'time to delay stream watcher operation'),
    ('rate', '',
        'video rate'),
    ('prefetchT', '',
        'time to delay stream watcher verfiy_vod_rate()'),
    ('server', '',
        'server path'),
    ('out_dir', '',
        'output directory'),
    ('order', '',
        'how many ran before me'),
    ('gap', '',
        'gap in prefetchT units between spawns'),
    ('verbose', '',
        'do output'),
    ('group_size', '',
        'group size'),
...
def parse_params(params, presets = {}):
    if len(params) == 0:
        return None
    config, args = parseargs(params, defaults, 0, 4, presets = presets)
    if args:
...
#            self.upmeasure.update_rate(x)
    ...
    def startEngine(self, ratelimiter = None, statusfunc = None):
        ...
        self.picker.setAttributes(self.streamwatcher.getCurrInterval,
                                  self.config,
                                  self.storagewrapper.piece_size,
                                  self.downmeasure)
        #######################
    ...
    def autoStats(self, displayfunc = None):
        ...
        DownloaderFeedback(self.choker, self.httpdownloader, self.rawserver.add_task,
            ...
            self.streamwatcher,self.downloader.picker,displayfunc, self.config['display_interval'])
    ...
    def startStats(self):     
        ...
        d = DownloaderFeedback(self.choker, self.httpdownloader, self.rawserver.add_task,
            self.upmeasure.get_rate, self.downmeasure.get_rate,
            self.ratemeasure, self.storagewrapper.get_stats,
            self.datalength, self.finflag,self.streamwatcher,self.downloader.picker, self.spewflag, self.statistics,self)
        return d.gather
